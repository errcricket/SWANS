...
min_feature_threshold = args[9]
max_feature_threshold =args[10]
...
def make_file(sample_file, project, organism):
	...
	with open('src/scripts/create_merged_dataset.R', 'w') as output_file:
		...
		for s in sample_list:
			...
			#seurat_string2 = '\t' + sample + '  <- CreateSeuratObject(counts=' + sample + ', project=\'' + project + '\', min.cells=3, min.features=' + min_feature_threshold + ')\n'
			...
			seurat_string3a =  '\t' + sample + ' <- AddMetaData(' + sample + ', metadata=\'' + s + '\', col.name=\'Samples\')\n\n'

			output_file.write(seurat_string + seurat_string1 + seurat_string2 + seurat_string3 + seurat_string3a)
			...
			seurat_string5 = '\t' + sample + '[[\'percent.mito\']] <- PercentageFeatureSet(S1, pattern = \'' + mito + '\')\n\n'

			s3 = '\tpdf(file=f1, onefile=TRUE, width=11, height=8.5)\n'
			s4 = '\tprint(VlnPlot(S1, features = c(\'nFeature_RNA\', \'nCount_RNA\', \'percent.mito\'), ncol = 3))\n'
			s4a = '\tplot1 <- FeatureScatter(S1, feature1 = \'nCount_RNA\', feature2 = \'percent.mito\')\n'
			s4b = '\tplot2 <- FeatureScatter(S1, feature1 = \'nCount_RNA\', feature2 = \'nFeature_RNA\')\n'
			s4c = '\tprint(plot1 + plot2)\n'
			s5 = '\tdev.off()\n\n'

			seurat_string6 = '\tS <- subset(S1, subset = nFeature_RNA > ' + min_feature_threshold + ' & nFeature_RNA < ' + max_feature_threshold + ' & percent.mito < ' + mito_cutoff + ')\n\n'
